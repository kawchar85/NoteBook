...
struct wavelet_tree {
  ...
  if (y - x + 1 < k || x > y)return unfound;
    if (l == r) { sl += tree[d][l]; return tree[d][l];
  ...
  Int ceil(int x, int y, Int k, int l=1, int r=n, int d=0){
    ...
    } else { tie(nx, ny) = right({x, y}, d, r);
        ans = min(ans, ceil(nx, ny, k, m + 1, r, d + 1));
    } return ans;  } } wvt;
main(){ cin >> n; cin >> wvt[i]; wvt.init(n);
cout << wvt.kth(5, 7, 2);cout << wvt.sl;
wvt.sl = 0;
cout << wvt.leq(5, 7, 70);cout << wvt.sl; }
